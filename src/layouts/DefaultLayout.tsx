                ...
                <SidebarInset className="flex h-full min-w-0 flex-col bg-background">
                    ...
                    <div className="shrink-0 p-2">
                        <header
                                    ...
                                    <BreadcrumbList className="flex flex-wrap gap-1 sm:hidden">
                                        {breadcrumbItems.slice(-1).map((item, index) => (
                                            <BreadcrumbItem key={`item-${index}`} className="min-w-0">
                                                <BreadcrumbPage
                                                    className="max-w-37.5 truncate font-semibold text-primary">
                                                    {item.title}
                                                </BreadcrumbPage>
                                            </BreadcrumbItem>
                                        ))}
                                            ...
                                            [
                                                <BreadcrumbItem key={`item-${index}`} className="min-w-0">
                                                    {item.isActive ? (
                                                        ...
                                                        <span
                                                            className="font-medium text-secondary text-sm">{item.title}</span>
                                                    )}
                                                </BreadcrumbItem>,
                            ...
                            <div
                                className="pointer-events-none absolute inset-0 rounded-lg bg-linear-to-r from-(--card-bg)/30 via-transparent to-(--card-bg)/30"/>
                        </header>
                    </div>
                        ...
                        >
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    ...
                                    <main
                                        className="flex max-h-[calc(100vh-5rem)] min-w-0 flex-1 flex-col overflow-y-auto overflow-x-hidden rounded-lg sm:h-[calc(100vh-6rem)]">
                                        <Outlet/>
                                    </main>
                                </motion.div>
                            </AnimatePresence>
