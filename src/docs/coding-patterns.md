...
### Real Example: Transit-AT

```
src/features/new-menu/transit-at/
├── components/
...
│       └── TabBroker.tsx
├── hooks/
│   └── use-transit.ts           # React Query hooks
├── services/
│   └── transit.service.ts
├── validations/
│   ├── transit.schemas.ts
...
export const [feature]Service = {
  ...
  getList: async (filter: [Feature]Filter): Promise<[Feature]Response> => {
    const { data } = await axios.post<[Feature]Response>(
      '/api/[endpoint]/list',
      filter
    );
    return data;
  ...
  getById: async (id: string): Promise<[Feature]> => {
    const { data } = await axios.get<[Feature]>(`/api/[endpoint]/${id}`);
    return data;
  },

  // POST - Create
  create: async (input: Create[Feature]Input): Promise<[Feature]> => {
    const { data } = await axios.post<[Feature]>('/api/[endpoint]', input);
    return data;
  },

  // PUT - Update
  update: async (id: string, input: Update[Feature]Input): Promise<[Feature]> => {
    const { data } = await axios.put<[Feature]>(
      `/api/[endpoint]/${id}`,
      input
    );
    return data;
  ...
  // Custom endpoints
  export: async (filter: [Feature]Filter): Promise<Blob> => {
    const { data } = await axios.post('/api/[endpoint]/export', filter, {
      responseType: 'blob'
    });
    return data;
  }
...
### Real Example: Transit Service

```typescript
// features/new-menu/transit-at/services/transit.service.ts
import axios from 'utils/axios';
import type { TransitFilter, TransitResponse, TransitDeclaration } from '../types';

export const transitService = {
  getDeclarations: async (filter: TransitFilter): Promise<TransitResponse> => {
    const { data } = await axios.post<TransitResponse>('/api/transit/declarations', filter);
    return data;
  },

  getById: async (id: string): Promise<TransitDeclaration> => {
    const { data } = await axios.get<TransitDeclaration>(`/api/transit/${id}`);
    return data;
  },

  create: async (input: Partial<TransitDeclaration>): Promise<TransitDeclaration> => {
    const { data } = await axios.post<TransitDeclaration>('/api/transit', input);
    return data;
  },

  update: async (id: string, input: Partial<TransitDeclaration>): Promise<TransitDeclaration> => {
    const { data } = await axios.put<TransitDeclaration>(`/api/transit/${id}`, input);
    return data;
  },

  delete: async (id: string): Promise<void> => {
    await axios.delete(`/api/transit/${id}`);
  },

  export: async (filter: TransitFilter): Promise<Blob> => {
    const { data } = await axios.post('/api/transit/export', filter, {
      responseType: 'blob'
    });
    return data;
  }
