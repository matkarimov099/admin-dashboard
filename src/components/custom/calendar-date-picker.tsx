				...
				<Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
					<PopoverTrigger asChild>
						<Button
							...
							variant={variant}
							className={cn(
								'relative w-full justify-start pr-8 text-left font-normal md:w-auto',
								className
							)}
							size="default"
						...
						>
							...
							{allowClear && date?.from && (
								// biome-ignore lint/a11y/useSemanticElements: Cannot use <button> inside <Button> component - would cause nested button elements which is invalid HTML
								<div
									role="button"
									tabIndex={0}
									...
									className="absolute top-1/2 right-2 z-10 h-4 w-4 -translate-y-1/2 cursor-pointer rounded-sm bg-background opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
									aria-label={'Clear'}
								>
									<X className="h-3 w-3" />
								</div>
					...
					{isPopoverOpen && (
						<PopoverContent
							className={cn('var(--radix-popover-trigger-weight) mx-3 p-0 md:w-fit')}
							align="center"
						...
						>
							<div
								className={cn(
									'flex',
									numberOfMonths === 2
										? 'flex-col gap-2 p-2 sm:flex-row sm:gap-4 sm:p-4'
										: 'flex-col p-4'
								)}
							>
								{numberOfMonths === 2 && (
									<div className="grid min-w-0 grid-cols-2 gap-1 border-foreground/10 border-b pr-0 pb-2 text-left sm:flex sm:min-w-[180px] sm:flex-col sm:border-r sm:border-b-0 sm:pr-4 sm:pb-0">
										{dateRanges.map(({ label, start, end }) => (
											<Button
												...
												size="sm"
												className={cn(
													'justify-start text-sm hover:bg-primary hover:text-primary-foreground hover:shadow-sm',
													selectedRange === label &&
								...
								<div className="flex flex-1 flex-col">
									...
									>
										<div className="flex w-full justify-center gap-2 sm:w-auto sm:justify-start">
											...
											>
												<SelectTrigger className="w-full font-medium hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 sm:w-fit">
