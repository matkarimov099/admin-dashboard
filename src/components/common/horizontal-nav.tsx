...
export function HorizontalNav() {
            ...
            if (!item.url) {
                ...
                if (item.items?.length) {
                    return item.items.some(
                        sub => !sub.roles?.length || hasRole(sub.roles)
                    );
                }
                return true;
            }
    ...
    return (
        <div className="flex items-center w-full">
            ...
            <nav
                ref={containerRef}
                className="hidden md:flex flex-1 items-center justify-center gap-1 min-w-0 overflow-hidden"
            >
                    ...
                    return (
                        item.items && item.items.length > 0 ? (
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button
                                        variant="ghost"
                                        size="md"
                                        className="h-11 px-4 shrink-0 text-[15px] font-[600]"
                                    >
                                        {t(item.title)}
                            ...
                            <Button
                                ...
                                size="md"
                                className={cn(
                                    ' px-4 shrink-0 font-medium text-[15px]',
                                    isActive && 'bg-(--color-primary) text-white font-bold'
                                )}
                            >
                                <NavLink
                                    to={item.url || '#'}
