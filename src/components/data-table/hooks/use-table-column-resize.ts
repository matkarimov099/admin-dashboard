 ...
 */
export function useTableColumnResize(tableId: string, enableResizing = false) {
	// Column sizing state
	const [columnSizing, setColumnSizing] = useState<ColumnSizingState>({});

	// Track if an initial load from localStorage is complete
	const initialLoadComplete = useRef(false);

	// Track if sizes have been changed by the user (to avoid overwriting on an initial load)
