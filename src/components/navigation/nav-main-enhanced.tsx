 ...
 */
function renderMenuItem(item: EnhancedMenuItemConfig | MenuGroupConfig, isCollapsed: boolean) {
  // Handle groups
  if (item.type === 'group') {
    return <NavGroup key={item.id} group={item as MenuGroupConfig} level={0} />;
  }
  ...
  return (
    <SidebarGroup key={item.id} className="mb-0 py-0">
      <SidebarMenu className="space-y-0.5 px-0">
        {renderMenuItemByType(item as EnhancedMenuItemConfig, 0)}
      </SidebarMenu>
    </SidebarGroup>
  );
}
 ...
 */
function renderMenuItemByType(item: EnhancedMenuItemConfig, level: number) {
  switch (item.type) {
    case 'collapse':
