...
export const ColorPicker = ({
  ...
  const [saturation, setSaturation] = useState(() => {
    if (selectedColor) {
      return selectedColor.saturationl() || 100;
    }
  ...
  const [lightness, setLightness] = useState(() => {
    if (selectedColor) {
      return selectedColor.lightness() || 50;
    }
  ...
  const [alpha, setAlpha] = useState(() => {
    if (selectedColor) {
      return selectedColor.alpha() * 100;
    }
    return defaultColor.alpha() * 100;
  });
...
// ColorPickerTrigger - Popover wrapper for easy usage
...
export const ColorPickerTrigger = ({
  ...
  const handleColorChange = (rgba: Parameters<typeof Color.rgb>[0]) => {
    if (onChange) {
  ...
  if (isDesktop) {
    return (
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-full max-w-sm p-3" align="start" sideOffset={4}>
          {pickerContent}
        </PopoverContent>
      </Popover>
    );
      ...
      <Drawer open={open} onOpenChange={setOpen}>
        <DrawerContent>
          <DrawerHeader>
            ...
            <DrawerDescription>Select a color for the degree badge</DrawerDescription>
          </DrawerHeader>
